...
#define NETLINK_TEST_BFDH 25 //自定义的协议
...
int main(int argc, char* argv[])
{
    ...
    int state_smg = 0;
    // Create a socket
    sock_fd = socket(AF_NETLINK, SOCK_RAW, NETLINK_TEST_BFDH);	//SOCK_RAW 套接字通信类型  RAW类型，提供原始网络协议访问  AF_NETLINK  网络通信的域
    if(sock_fd == -1){
    ...
    while(1){
		...
		system("cd /mnt1");
		system("mplayer -ac mad -framedrop -fs 03.avi");
    }
    close(sock_fd);
    return 0;
}
